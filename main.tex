...
The Land Price Estimator system is developed using the \textbf{Django} web framework, a high-level Python framework based on the \textbf{Model--View--Template (MVT)} architecture. Django was selected for its rapid development capabilities, strong security features, and suitability for data-driven web applications.

Django handles request processing, business logic, database operations, and dynamic page rendering. The system uses \textbf{server-side rendering} with Django templates, where HTML pages are generated on the server to ensure secure processing and consistent user experience.

Database interactions are managed through Django’s \textbf{Object-Relational Mapping (ORM)}, allowing models to represent database tables and reducing reliance on raw SQL queries. This improves maintainability and helps prevent common security risks.

User authentication and authorization are implemented using Django’s authentication system with a \textbf{custom user model} to support multiple roles, including normal users, data scientists, and administrators. Access control is enforced using login decorators and role-based redirection.

The project follows a \textbf{modular multi-application structure}, where each major user role is implemented as a separate Django app, improving scalability and code organization. Overall, Django provides a secure, structured, and efficient foundation for the system.
...
\begin{figure}[H]
  \centering
  \caption{Figure 4.4.1 Normal user side app structure}
  \includegraphics[width=0.4\linewidth]{images/Figure 4.4.2 Normal user side app structure.png}
\end{figure}
...
\includegraphics[width=0.6\textwidth]{images/Figure 4.7.7 CSRF token usage example.png}
...
\includegraphics[width=0.5\textwidth]{images/Figure 5.3.5 Save and Estimate buttons.png}
