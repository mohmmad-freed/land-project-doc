...
% 2) في قائمة الأشكال (List of Figures): إخفاء رقم الشكل التلقائي
\renewcommand{\cftfigpresnum}{}   % لا تطبع الرقم قبل العنوان
\renewcommand{\cftfigaftersnum}{} % ولا أي فاصل بعده
\setlength{\cftfignumwidth}{0pt}  % عدم حجز عرض لرقم الشكل
\renewcommand{\cftfigleader}{\cftdotfill{\cftdotsep}} % تأكيد نقاط القيادة

% 3) في قائمة الجداول (List of Tables): إخفاء رقم الجدول التلقائي
...
\section{Overview}
This chapter describes the implementation of the Land Price Estimator system. It explains how the frontend and backend components were developed, how the database is used to store and manage data, and how different parts of the system interact with each other. The chapter also presents the security measures applied during development and discusses the current limitations of the system and possible future improvements.
...
The Land Price Estimator system is built using a combination of programming and markup languages to handle both the backend and frontend functionalities efficiently. The chosen languages and their roles are as follows:

\begin{itemize}[leftmargin=*, itemsep=3pt]
    \item \textbf{Python:} Python is the main programming language used for the backend of the system. It is leveraged primarily through the Django framework to implement the Model–View–Template (MVT) architecture, handle business logic, manage the database via the ORM, and process user requests.

    \item \textbf{HTML (HyperText Markup Language):} HTML is used to structure the content of the web pages. It defines elements such as forms, links, buttons, and headings, providing the foundation for all frontend interfaces seen by the users.

    \item \textbf{CSS (Cascading Style Sheets):} CSS is used to style HTML elements, including layout, colors, fonts, spacing, and responsiveness. It ensures a visually appealing and consistent interface across different devices and browsers.

    \item \textbf{JavaScript:} JavaScript is used to implement interactive features on the frontend. This includes dynamic form validation, responsive components, and user interface enhancements that improve the usability and responsiveness of the web application.

    \item \textbf{Tailwind CSS:} Tailwind CSS is used alongside standard CSS to streamline styling and maintain consistency. It allows developers to apply pre-defined classes directly in HTML templates, speeding up the development process and making the interface highly customizable without writing extensive custom CSS.
\end{itemize}


\subsection{Web Framework}

The Land Price Estimator system is developed using the \textbf{Django} web framework. Django is a high-level Python web framework that follows the \textbf{Model--View--Template (MVT)} architectural pattern and is designed to support rapid development, scalability, and secure web-based applications.

Django serves as the core backend framework of the system and is responsible for handling HTTP requests, implementing business logic, managing database interactions, and rendering dynamic web pages. The MVT architecture is clearly reflected in the project structure, where models define the data schema, views contain the application logic, and templates generate the user interface presented to the users.

The system adopts a \textbf{server-side rendering} approach using Django’s template engine. Dynamic content is generated on the server and delivered to the client as fully rendered HTML pages, ensuring better security, simplified logic flow, and compatibility across different browsers and devices.

Django’s built-in \textbf{Object-Relational Mapping (ORM)} is used to interact with the database using Python objects instead of raw SQL queries. This abstraction improves code readability, maintainability, and reduces the risk of SQL injection vulnerabilities.

User authentication and authorization are implemented using Django’s authentication framework with a \textbf{custom user model}. This design allows the system to support multiple user roles, including normal users, data scientists, and administrators. Role-based access control is enforced using decorators such as \texttt{@login\_required}, and users are redirected to the appropriate application section based on their account type after successful authentication.

Django Forms and ModelForms are used to handle user input, perform server-side validation, manage file uploads, and securely process sensitive information such as passwords. Password management relies on Django’s built-in hashing mechanisms, ensuring that user credentials are stored and handled securely.

The project follows a \textbf{modular multi-application structure}, where each user role is implemented as a separate Django application. This approach improves code organization, scalability, and maintainability by clearly separating concerns between different system components.

Overall, Django was selected for this project due to its robust security features, clear architectural design, built-in authentication system, and strong support for data-driven web applications.


\subsection{Development Tools}
\begin{itemize}[leftmargin=*, itemsep=3pt]
    \item \textbf{Integrated Development Environment (IDE):} Visual Studio Code (VS Code) was used as the primary integrated development environment. It provides strong support for Python and Django development, including syntax highlighting, debugging tools, code formatting, and extension support. Its flexibility and lightweight nature made it suitable for managing both backend and frontend code within the same environment.

    \item \textbf{Version Control System:} Git was used for version control to track changes in the project source code, manage development history, and support collaboration between team members. The project repository was hosted on GitHub, which enabled centralized code management, backup, and easy sharing of the project.

    \item \textbf{Database Management:} During development, SQLite was used as the default database system provided by Django. It is lightweight and easy to configure. Django’s Object-Relational Mapping (ORM) was used to interact with the database without writing raw SQL queries.

    \item \textbf{Web Browser and Testing Tools:} Modern web browsers such as Brave and Comet were used for testing and debugging the frontend interface. Built-in developer tools were utilized to inspect HTML elements, debug JavaScript behavior, and test responsiveness across different screen sizes.

    \item \textbf{Machine Learning Development Environment:} Google Colab was used to develop, train, and validate the machine learning model for predicting land prices. Using Python libraries such as scikit-learn, the Decision Tree Regression algorithm was implemented and trained on historical land parcel datasets. After model training, the resulting trained model was exported and later integrated into the Django backend. This allows the system to receive user-submitted land parcel data through a form, pass it to the trained model, and return predicted land prices in real time.
\end{itemize}

\subsection{Supporting Libraries}
In addition to the main frameworks and development tools, several supporting libraries were used to facilitate backend development, form handling, authentication, and machine learning model training.

\begin{itemize}[leftmargin=*, itemsep=3pt]
    \item \textbf{Django Built-in Libraries:} The system relies  on Django’s built-in libraries to implement core backend functionalities. These include modules for authentication, authorization, form handling, and request processing. Django’s authentication utilities were used to manage user login, logout, and access control, ensuring that only authorized users can access specific system features.

    \item \textbf{Django Forms:} Django’s ModelForm system was used to generate and manage forms based on database models. This approach simplifies form creation, enforces data validation at the server level, and ensures consistency between user input and stored data. 

    \item \textbf{Machine Learning Libraries:} For machine learning model development, scikit-learn (imported using the sklearn module) was used to implement the Decision Tree Regression algorithm. The train\_test\_split function was utilized to divide the dataset into training and testing subsets, enabling proper model evaluation.

    \item \textbf{Data Processing Library:} Pandas was used for data manipulation and preprocessing during the machine learning development phase. It was employed to load datasets, handle missing values, and prepare structured data suitable for training the regression model.
